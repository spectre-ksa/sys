...
client.on('guildMemberAdd', member => {
     const welcomer =  member.guild.channels.find('name', 'welcom');
    if(!welcomer) return;
      if(welcomer) {
         moment.locale('ar-ly');
         var m = member.user;
    ...
    .setFooter('Requested by '+message.author.username, message.author.avatarURL)
      ...
      if( msg.toLowerCase().startsWith( prefix + 'roleembed' ) ){
          ...
          if( !role1 ) return  message.channel.send( '**:x: يرجى وضع الرتبة المراد اعطاءها الى الشخص**' );if( message.mentions.members.first() ){
              ...
              return message.channel.send('**:white_check_mark: Changed role for '+args[0]+', +'+role1.name+'**');
          }
...
if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.channel.send("**I Don't Have `MANAGE_ROLES` Permission**").then(msg => msg.delete(6000))
...
setTimeout(function(){
      ...
      message.channel.send(`<@${tomute.id}> **The time passed and the mute was disconnected from the person**:white_check_mark: `);
    }, ms(mutetime));
...
if(command === `unmute`) {
  if(!message.member.hasPermission("MANAGE_ROLES")) return message.channel.sendMessage("**You do not have permission to unselect the person who Muted**:x: ").then(m => m.delete(5000));
if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("**I Don't Have `MANAGE_ROLES` Permission**").then(msg => msg.delete(6000))
  ...
  if(!role || !toMute.roles.has(role.id)) return message.channel.sendMessage("**This person was not Muted*:x:")
 
  await toMute.removeRole(role)
  message.channel.sendMessage("**I have been unmuted the person successfully**:white_check_mark:");
...
3) حاول انك ما تسب احد..
4) لا تكون عنصري ولا تتحرش بأحد بطريقة وصخة...
5) لا تضحك ع أشياء حساسة..
6) لا تتكلم عن السياسة ا شيء ديني او حساس
7) أحترم الأخرين وفكر باللي تقوله
8) لا تتحرش بالناس ولا تكون شخص متنمر
9) لا ترسل روابط مواقع او اي شيء اباحي 
10) ممنوع السبام
●▬▬▬▬▬▬▬▬▬▬▬▬▬ஜ۩۞۩ஜ▬▬▬▬▬▬▬▬▬▬▬▬▬●
